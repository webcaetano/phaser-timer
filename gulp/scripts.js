...
module.exports = function(options) {
	...
	gulp.task('scripts', function () {
		return wp(false, true, options.src + '/index.js',options.tmp + '/serve/scripts', false);
	});

	gulp.task('scripts:dependent', function () {
		return wp(true, true, options.src + '/index.js',options.tmp + '/serve/scripts', false);
	});

	gulp.task('scripts:watch', function (callback) {
		return wp(false, true, options.src + '/index.js',options.tmp + '/serve/scripts', true, callback, true);
	});

	gulp.task('scripts:test', function () {
		return wp(false, false, 'test/scripts/index.js',options.tmp + '/serve/test', false);
	});

	gulp.task('scripts:test:watch', function (callback) {
		return wp(false, false, 'test/scripts/index.js',options.tmp + '/serve/test', true, callback, true);
	});
};
